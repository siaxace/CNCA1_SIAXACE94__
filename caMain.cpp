...
int main(int argn, char** args)
{
	...
	crypto ramznegar;
	string capu, capr, caph="salam";

	ramznegar.genKey(caph, capr, capu);
	write2File_entire(ca_pubkey_adr,capu);
				...
				else
				{
					...
					else
					{
						if( string(buff_read) != "DC")
						{
							...
							serverReply = ramznegar.signedOf(clientInput, capr, caph);
